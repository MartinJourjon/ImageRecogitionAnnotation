...
async function save() {
  ...
  const { error } = await annotationsService.updateAnnotation(props.record.img_id, payload)
  saving.value = false
  if (error) {
...
async function deleteAsNotHuman() {
  ...
  deleting.value = true
  errorMsg.value = ''
  const { error } = await annotationsService.deleteAnnotation(props.record.img_id)
...
async function skip() {
  ...
  errorMsg.value = ''
  try {
    const { error } = await annotationsService.skipAnnotation(props.record.img_id)
    if (error) {
      errorMsg.value = error
      return
    }
